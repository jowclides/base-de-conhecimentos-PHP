		...
		if (strval($this->CurrentUserID) == "") {
			...
			if (Config("USER_ID_IS_HIERARCHICAL")) {
				...
				while ($userIDList != $curUserIDList) {
					$filter = '`senha` IN (' . $curUserIDList . ')';
					$sql = $UserTable->getSql($filter);
					if ($rsUser = Conn($UserTable->Dbid)->execute($sql)) {
						while (!$rsUser->EOF) {
							$this->addUserID($rsUser->fields('email'));
							$rsUser->moveNext();
						}
						$rsUser->close();
					}
	...
	public function parentUserIDList($userId)
	{
		...
		if (SameString($userId, CurrentUserID())) {
			if (strval(CurrentParentUserID()) != "")
				$result = QuotedValue(CurrentParentUserID(), DATATYPE_STRING, Config("USER_TABLE_DBID"));
			return $result;
		}

		// One level only, must be CurrentUserID
		if (!Config("USER_ID_IS_HIERARCHICAL")) {
			return QuotedValue(CurrentUserID(), DATATYPE_STRING, Config("USER_TABLE_DBID"));
			...
			for ($i = 0; $i < $len; $i++) {
				if (strval($ar[$i]) != trim(strval($userId))) {
					if ($result != "")
						$result .= ", ";
					$result .= QuotedValue($ar[$i], DATATYPE_STRING, Config("USER_TABLE_DBID"));
				}
			}
			return $result;
		}
